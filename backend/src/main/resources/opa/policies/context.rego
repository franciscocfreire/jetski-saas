...
deny_horario contains msg if {
    input.action in ["locacao:checkin", "locacao:checkout", "abastecimento:registrar"]
...
allow_device_mobile if {
    input.action in ["locacao:checkin", "locacao:checkout", "foto:upload"]
...
deny_ambiente contains msg if {
    input.action in ["fechamento:mensal", "comissao:aprovar"]

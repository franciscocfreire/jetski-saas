...
class TenantContextTest {
    ...
    void shouldSetAndGetUserRoles() {
        // Given
        var roles = java.util.List.of("GERENTE", "OPERADOR");

        // When
        TenantContext.setUserRoles(roles);

        // Then
        assertThat(TenantContext.getUserRoles()).containsExactlyElementsOf(roles);
    }

    @Test
    void shouldReturnEmptyListWhenUserRolesNotSet() {
        // When / Then
        assertThat(TenantContext.getUserRoles()).isEmpty();
    }

    @Test
    void shouldClearUserRolesWhenClearing() {
        // Given
        UUID tenantId = UUID.randomUUID();
        var roles = java.util.List.of("ADMIN_TENANT");
        TenantContext.setTenantId(tenantId);
        TenantContext.setUserRoles(roles);

/**
 * Rede de emissão (EMISSAO_DELEGADA_SPEC §8.M): cada delegada aparece logo abaixo da
 * EAMA emissora dela, quando a EAMA também está na lista. Delegada cuja EAMA não está
 * na lista fica como raiz (a tela mostra "delegada de …").
 */
...
export function ordenarPorRede<T extends EmpresaNaRede>(empresas: T[]): { empresa: T; nivel: 0 | 1 }[] {
  ...
  for (const raiz of raizes) {
    ...
    for (const filha of doRaiz) {
      resultado.push({ empresa: filha, nivel: 1 })
    }
  }
  return resultado
}

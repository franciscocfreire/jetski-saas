...
export function StepDocumentos({
  ...
  const salvarEndereco = useMutation({
    mutationFn: () =>
      clientesService.update(atendimento.cliente!.id, {
    ...
    onError: () => toast.error('Falha ao salvar o endereço.'),
  })
  ...
  function avancar() {
    ...
    // Endereço só precisa ir ao backend quando preenchido (Declaração 1-C).
    if (endereco?.logradouro) salvarEndereco.mutate()
    else onDone({ endereco, temComprovanteResidencia: temComprovante, temCha })
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between">
        ...
        <Button type="button" disabled={salvarEndereco.isPending} onClick={avancar}>
          {salvarEndereco.isPending ? 'Salvando…' : 'Avançar'}
        </Button>
      </div>
    </div>
  )

...
POSTGRES_PORT="5432"  # Docker Compose PostgreSQL
...
if ! PGPASSWORD=dev123 psql -h $POSTGRES_HOST -p $POSTGRES_PORT -U postgres -c '\q' 2>/dev/null; then
...
PGPASSWORD=dev123 psql -h $POSTGRES_HOST -p $POSTGRES_PORT -U postgres <<EOF >/dev/null 2>&1 || true
DO \$\$
BEGIN

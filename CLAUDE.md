...
3. **Identidade única por pessoa** (decisão de jul/2026 — revoga a regra anterior de "duas populações que nunca se cruzam"): uma pessoa = uma identidade, que **acumula papéis** — staff (`Membro`) de N empresas, cliente de N lojas e/ou operador de plataforma (`usuario_global_roles`). Ser cliente não impede ser staff nem operador, e vice-versa. O que continua proibido é **vínculo automático por coincidência de e-mail**: todo link identidade↔papel é explícito e auditado (ação de admin ou identidade verificada pelo IdP), nunca JIT cego. *Unificação CONCLUÍDA (F0–F4, 30/jul/2026, `IDENTIDADE_UNICA_SPEC.md`)*: `usuario` é a raiz única da pessoa; o vínculo comercial é `cliente.usuario_id` (unique parcial por tenant, self-read pela policy `cliente_self_read`); `cliente_identity_provider` NÃO EXISTE MAIS. Regra de produto: o cliente é da PLATAFORMA; a loja só vê a ficha `cliente` depois de existir negócio.
...
- **Rotas de plataforma** (`/v1/platform/**`): `X-Tenant-Id` é **opcional** (o console não tem empresa corrente; o alvo vai no path) e o `PlatformScopeInterceptor` nega quem não tem papel `PLATFORM_*` **antes** do OPA. Ações OPA usam o path completo (`platform:tenants:approve`), não o último segmento — e a decisão considera o **método HTTP** (`input.context.method`), porque leitura e escrita colapsam na mesma ação em várias rotas.
- **Operador de plataforma**: `unrestricted_access` = **alcance** (enxerga qualquer empresa); o **poder** vem do papel em `usuario_global_roles.roles[]` (`PLATFORM_ADMIN`/`SUPORTE`/`FINANCEIRO`/`LEITURA`), matriz em `policies/authz/platform.rego` + enum `PapelPlataforma`. Concessão pela tela `/operadores` do console (não mais só `PLATFORM_ADMIN_EMAILS`/SQL).

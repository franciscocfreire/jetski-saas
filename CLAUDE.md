...
3. **Identidade única por pessoa** (decisão de jul/2026 — revoga a regra anterior de "duas populações que nunca se cruzam"): uma pessoa = uma identidade, que **acumula papéis** — staff (`Membro`) de N empresas, cliente de N lojas e/ou operador de plataforma (`usuario_global_roles`). Ser cliente não impede ser staff nem operador, e vice-versa. O que continua proibido é **vínculo automático por coincidência de e-mail**: todo link identidade↔papel é explícito e auditado (ação de admin ou identidade verificada pelo IdP), nunca JIT cego. *Migração em andamento*: `cliente_identity_provider` (por tenant) ainda é paralelo ao `usuario_identity_provider` (global) — unificar é projeto próprio, spec em `IDENTIDADE_UNICA_SPEC.md` (regra de produto: o cliente é da PLATAFORMA; a loja só vê a ficha `cliente` depois de existir negócio).
